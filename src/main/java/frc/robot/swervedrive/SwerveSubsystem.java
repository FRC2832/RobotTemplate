...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  ///select what motor we are to set the feed forward correctly.  numMotors should always be 1, it will be expanded inside swerveDrive
  private DCMotor motorFeedForward = DCMotor.getNEO(1);
  ...
  public Rotation2d getTargetYaw(Pose3d target) {
    return getTargetYaw(target.toPose2d());
  }

  public Rotation2d getTargetYaw(Pose2d target) {
    Translation2d relativeTrl = target.relativeTo(getPose()).getTranslation();
   ...
   */
  public Command aimAtTarget(Pose3d target, double tolerance)
  {
    ...
    return run(
        () -> {
          ChassisSpeeds speeds = new ChassisSpeeds(0, 0,
                                                   ...
                                                   getTargetYaw(target).getRadians()));
          speeds.toRobotRelativeSpeeds(getHeading());
          drive(speeds);
        }).until(() -> Math.abs(getTargetYaw(target).minus(getHeading()).getDegrees()) < tolerance);
  }
  ...
  */
  ...
  {
    swerveDrive.setDriveMotorModel(motorFeedForward);
    try
    {
